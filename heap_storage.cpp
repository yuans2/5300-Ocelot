...
void SlottedPage::put(RecordID record_id, const Dbt &data) throw(DbBlockNoRoomError)
{
	...
	get_header(old_size, old_size, record_id);
	...
	if (new_size > old_size)
	{
		...
		shift_record(record_id + 1, shift_offset);
		memcpy(this->address(new_offset), data.get_data(), new_size);
		put_header(i, new_size, new_offset);	
	...
	else
	{
		...
		shift_record(record_id + 1, shift_offset, false);
		memcpy(this->address(new_offset), data.get_data(), new_size);
		put_header(i, new_size, new_offset);	
...
void SlottedPage::shift_records(RecordID begin_record_id, u_int16_t shift_offset, bool left = true)
{
